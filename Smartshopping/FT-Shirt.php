...
$sql = "SELECT COUNT(*) FROM items where category='Tshirts' AND gender='F' ";
$result = select($sql);
...
$sql = "SELECT itemid, name, category, brand, color, price, path FROM items WHERE category='Tshirts' AND gender='F' LIMIT $offset, $rowsperpage";
$result = select($sql);
$i=1;
...
while ($list = mysql_fetch_assoc($result)) {
   ...
   $item_id=$list['itemid'];
                ...
                echo "<h3>'$image_brand'</h3>" ;
                echo "<p class='product_price'>'$image_price'</p> ";
                echo "<a href='shoppingcart.html' class='add_to_cart'>Add to Cart</a> </div>";*/
